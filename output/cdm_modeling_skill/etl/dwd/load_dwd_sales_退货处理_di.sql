...
INSERT OVERWRITE TABLE dwd_sales_退货处理_di PARTITION (pt='${PT_DATE}')
SELECT
    source.退货处理_id,
    ROW_NUMBER() OVER (ORDER BY source.退货处理_id) AS 退货处理_sk,
    ...
    CASE
        WHEN  dim_shop.shop_sk IS NOT NULL
         ...
         AND   dim_customer.customer_sk IS NOT NULL
    ...
    'dws_sales_退货处理' AS source_system

FROM dws_sales_退货处理_di source
...
-- ========================================
SELECT
    'dwd_sales_退货处理_di' AS table_name,
    '${PT_DATE}' AS pt,
