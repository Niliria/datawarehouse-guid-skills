...
INSERT OVERWRITE TABLE dwd_customer_客户管理_di PARTITION (pt='${PT_DATE}')
SELECT
    source.客户管理_id,
    ROW_NUMBER() OVER (ORDER BY source.客户管理_id) AS 客户管理_sk,
    ...
    -- 业务标志
    CASE
        WHEN  dim_customer.customer_sk IS NOT NULL
    ...
    'dws_customer_客户管理' AS source_system

FROM dws_customer_客户管理_di source
...
-- ========================================
SELECT
    'dwd_customer_客户管理_di' AS table_name,
    '${PT_DATE}' AS pt,
    ...
    COUNT(DISTINCT customer_sk) AS customer_unique_count

FROM dwd_customer_客户管理_di
WHERE pt = '${PT_DATE}'
GROUP BY pt;