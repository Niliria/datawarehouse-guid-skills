...
INSERT OVERWRITE TABLE dwd_shop_店铺管理_di PARTITION (pt='${PT_DATE}')
SELECT
    source.店铺管理_id,
    ROW_NUMBER() OVER (ORDER BY source.店铺管理_id) AS 店铺管理_sk,
    ...
    'dws_shop_店铺管理' AS source_system

FROM dws_shop_店铺管理_di source
...
-- ========================================
SELECT
    'dwd_shop_店铺管理_di' AS table_name,
    '${PT_DATE}' AS pt,
