...
INSERT OVERWRITE TABLE dwd_product_商品管理_di PARTITION (pt='${PT_DATE}')
SELECT
    source.商品管理_id,
    ROW_NUMBER() OVER (ORDER BY source.商品管理_id) AS 商品管理_sk,
    ...
    -- 业务标志
    CASE
        WHEN  dim_product.product_sk IS NOT NULL
    ...
    'dws_product_商品管理' AS source_system

FROM dws_product_商品管理_di source
...
-- ========================================
SELECT
    'dwd_product_商品管理_di' AS table_name,
    '${PT_DATE}' AS pt,
    ...
    COUNT(DISTINCT product_sk) AS product_unique_count

FROM dwd_product_商品管理_di
WHERE pt = '${PT_DATE}'
GROUP BY pt;