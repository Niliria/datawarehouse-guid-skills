...
- `dwd_inv_inventory_snapshot_df`: process `inventory_snapshot`, grain `一行一个SKU在一个门店的库存记录`, fact type `periodic_snapshot`
- `dwd_mkt_coupon_usage_df`: process `coupon_usage`, grain `一行一次用户优惠券使用`, fact type `transaction`
- `dwd_trd_order_detail_df`: process `order_detail`, grain `一行一笔订单明细`, fact type `transaction`
...
- `dwd_trd_refund_df`: process `refund`, grain `一行一笔退款记录`, fact type `transaction`
