# 字段标签标注结果 - business_evaluation_supervision

## 一、表基本信息

| 项目 | 内容 |
|------|------|
| 表名 | business_evaluation_supervision |
| 库名 | yummy |
| 行数 | 0（空表） |
| 表注释 | 商务评分【监理类】 |
| 主题域 | 评价域 |
...
| 11 | bidding_contract_score | varchar(255) | [不可加度量] | 招标合同（20分） |
...
| 15 | payment_terms_score | varchar(255) | [不可加度量] | 付款条件（20分） |
| 16 | payment_terms_basis | varchar(255) | [维度属性] | 评分依据 |
| 17 | payment_request_score | varchar(255) | [不可加度量] | 请款（20分） |
| 18 | payment_request_basis | varchar(255) | [维度属性] | 评分依据 |
| 19 | business_contract_dispute_score | varchar(255) | [不可加度量] | 商务合同纠纷（20分） |
